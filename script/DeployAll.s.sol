...
contract DeployAll is Script {
    function run() external {
        ...
        IdentityGuard idGuard = new IdentityGuard(address(idRegistry));
        ...
        // ═══════════════════════════════════════════════════════════════════
        GovernanceHybrid gov = new GovernanceHybrid(address(etr), repOracle);
        ...
        VenueProgramRegistry venues = new VenueProgramRegistry(admin, address(etr));
        DJSetLedger sets = new DJSetLedger(admin, address(pass), address(venues));
        CulturalSignalRegistry signals = new CulturalSignalRegistry(admin);
        PerformanceAttribution attrib = new PerformanceAttribution(
            admin, address(sets), address(venues), address(signals), communityPool
        );
        ...
        HumanityUpgradeProtocol hup = new HumanityUpgradeProtocol(admin, address(etr));
        ...
        RewardsDistributor rewardsDist = new RewardsDistributor(
            admin,
            address(vibe),
            rewardDistributor, // use env REWARD_DISTRIBUTOR as signer
            1_000_000 ether    // 1M VIBE epoch cap
        );

        // Agent infra role wiring
